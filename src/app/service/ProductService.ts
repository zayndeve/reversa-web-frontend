...
class ProductService {
  ...
  public async getNewArrivals(input: ProductInquiry): Promise<Product[]> {
    try {
      let url = `${this.path}/api/product/new-arrivals?order=${input.order}&page=${input.page}&limit=${input.limit}`;
  ...
  /** Get Popular Products */
  public async getPopularProducts(input: ProductInquiry): Promise<Product[]> {
    try {
      let url = `${this.path}/api/product/popular?order=${input.order}&page=${input.page}&limit=${input.limit}`;
      if (input.productCategory)
        url += `&productCategory=${input.productCategory}`;
      if (input.search) url += `&search=${input.search}`;
